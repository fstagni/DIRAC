...
class StatesMonitoringAgent(AgentModule):
  ...
  def execute(self):
    ...
    if not result['OK']:
      ...
      if retVal['OK']:
        self.log.info("The records are successfully sent to the backend!")
      else:
        self.log.warn("Failed to insert the records! It will be retried in the next iteration", retVal['Message'])
