...
#
  ...
  def export_updateSoftware(self, version, rootPath="", gridVersion=""):
    ...
    if webPortal:
      ...
      webappCompileScript = os.path.join(
          gComponentInstaller.instancePath,
          'pro',
          "WebAppDIRAC/scripts",
  ...
  def __storeProfiling():
    ...
    # The "component name" (e.g. ElementInspectorAgent_Resource)
    # does not necessarily correspond to the "component module" (e.g. ElementInspectorAgent)
    # so the lines below trigger some errors on most of the machines.
    # The problem is in the "DB" schema itself which does not allow this separation
    # (or at least this is not seen in the Monitoring Web app).
